...
//    verifyConsumerGroups(Map.of(subgroup0, 1, subgroup1, 1));
    waitForConsumerGroupCount(2);

    var allConsumerGroups = getAllConsumerGroups(CIRCULATION_RULES_UPDATED.name());
    var groupId1 = allConsumerGroups.get(0);
    var groupId2 = allConsumerGroups.get(1);
    int initialOffsetForSubgroup0 = getOffsetForCirculationRulesUpdateEvents(groupId1);
    int initialOffsetForSubgroup1 = getOffsetForCirculationRulesUpdateEvents(groupId2);

    JsonObject rules = circulationRulesFixture.getRules().getJson();
    publishCirculationRulesUpdateEvent(rules, rules);

    waitForValue(() -> getOffsetForCirculationRulesUpdateEvents(groupId1), initialOffsetForSubgroup0 + 1);
    waitForValue(() -> getOffsetForCirculationRulesUpdateEvents(groupId2), initialOffsetForSubgroup1 + 1);
...
//    Rules originalCachedRules = getInstance().getRules(TENANT_ID);
//
//    waitForConsumerGroupCount(1);
//    String groupId = getAllConsumerGroups(CIRCULATION_RULES_UPDATED.name()).getFirst();
//    int initialOffset = getOffsetForCirculationRulesUpdateEvents(groupId);
//    publishEvent(CIRCULATION_RULES_TOPIC, event);
//    waitForValue(() -> getOffsetForCirculationRulesUpdateEvents(groupId), initialOffset + 1);
//
//    Rules newCachedRules = getInstance().getRules(TENANT_ID);
//    assertThat(newCachedRules.getRulesAsText(), equalTo(originalCachedRules.getRulesAsText()));
//    assertThat(newCachedRules.getReloadTimestamp(), equalTo(originalCachedRules.getReloadTimestamp()));
//  }
  ...
  @Test
  void circulationRulesUpdateEventConsumerSubgroupCreated() {
    // We don't need to run the first verticle, it's already running
    var secondVerticle = deployVerticle();
    var thirdVerticle = deployVerticle();
    waitForConsumerGroupCount(3);
    verifyNumberOfGroupsWithConsumers(3);

    undeployVerticle(secondVerticle);
    undeployVerticle(thirdVerticle);
    deployVerticle();
    waitForConsumerGroupCount(4);
    verifyNumberOfGroupsWithConsumers(2);
  }
  ...
  private static int getOffsetForCirculationRulesUpdateEvents(String groupId) {
    return getOffset(CIRCULATION_RULES_TOPIC, groupId);
  }

//  private static String buildConsumerSubgroupId(DomainEventType eventType, String uuid, long millis) {
//    return String.format("%s_%s_%d", buildConsumerGroupId(eventType), uuid, millis);
//  }
  ...
  @SneakyThrows
  private static List<String> getAllConsumerGroups(String topic) {
    return waitFor(kafkaAdminClient.listConsumerGroups()
      .map(consumers -> consumers.stream()
        .peek(consumer -> log.info("Consumer group: {}", consumer.getGroupId()))
        .map(ConsumerGroupListing::getGroupId)
        .filter(groupId -> groupId.startsWith(topic))
        .toList()));
  }
  ...
  private Map<String, ConsumerGroupDescription> verifyNumberOfGroupsWithConsumers(
    int groupsWithConsumers) {

    return waitAtMost(30, SECONDS)
      .until(() -> waitFor(
          kafkaAdminClient.describeConsumerGroups(
            new ArrayList<>(getAllConsumerGroups(CIRCULATION_RULES_UPDATED.name())))),
        groups -> groupsWithConsumers == groups.values()
          .stream()
          .map(consumerGroupDescription -> consumerGroupDescription.getMembers().size())
          .map(Math::signum)
