...
public class ItemsInTransitReportService {
  ...
  private CompletableFuture<Result<ItemsInTransitReportContext>> fetchHoldingsRecords(
    ...
    logger.info("[TRACE] -> items map size {}", context.getItems().isEmpty() ? 0 : context.getItems().size());
  ...
  private CompletableFuture<Result<ItemsInTransitReportContext>> fetchInstances(
    ...
    logger.info("[TRACE] -> holdings map size {}", context.getHoldingsRecords().isEmpty() ? 0 : context.getHoldingsRecords().size());
  ...
  private CompletableFuture<Result<ItemsInTransitReportContext>> fetchLocations(
    ...
    logger.info("[TRACE] -> instances map size {}", context.getInstances().isEmpty() ? 0 : context.getInstances().size());
